...
Считается не на глаз, а по данным caniuse: `npm run check:support` печатает
таблицу и говорит, какая именно фича задаёт границу.

| Браузер | Минимум | Что упирается |
| --- | --- | --- |
| Chrome, Edge | **84** (июль 2020) | `gap` во flex |
| Firefox | **63** (октябрь 2018) | `gap` во flex |
| Safari | **14.1** (апрель 2021) | `gap` во flex |
| Safari iOS | **14.5** | `gap` во flex |
| Samsung Internet | **14.0** | `gap` во flex |
| Internet Explorer | **не работает** | кастомные свойства |

Ниже этой границы система не «выглядит хуже», а разъезжается: `gap` держит
все отступы во flex-раскладке, замены у него нет.

**Что деградирует мягко** и границу не поднимает — для каждого пункта в css
объявлен фоллбэк, который старый браузер возьмёт вместо непонятого значения:
...
**Про IE11.** Он недостижим принципиально: вся система стоит на кастомных
свойствах, а темы и плотности — это их переопределение на контейнере. Для IE
нужна не настройка, а другая сборка — статический css одной темы с
подставленными значениями. Если такая задача появится, генератор для неё
есть, добавить нужно только режим вывода.

**Хотите ещё ниже?** Отказ от `gap` во flex (43 места, замена на margin)
опускает границу до Chrome 61 / Firefox 55 / Safari 10.1 — но это заметно
портит код раскладки, поэтому по умолчанию не сделано.
